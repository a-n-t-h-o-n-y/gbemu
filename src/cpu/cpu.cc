...
CPU::CPU(MMU& inMMU, Options& inOptions)
    : mmu(inMMU), options(inOptions), af(a, f), bc(b, c), de(d, e), hl(h, l)
{}
...
bool CPU::handle_interrupt(u8 interrupt_bit,
...
bool CPU::is_condition(Condition condition)
{
    ...
    switch (condition) {
        case Condition::C: should_branch = f.flag_carry(); break;
        case Condition::NC: should_branch = !f.flag_carry(); break;
        case Condition::Z: should_branch = f.flag_zero(); break;
        case Condition::NZ: should_branch = !f.flag_zero(); break;
    }
...
Cycles CPU::execute_normal_opcode(const u8 opcode, u16 opcode_pc)
{
    ...
    return !branch_taken ? opcode_cycles[opcode]
